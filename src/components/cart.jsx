...
const itemsVis = () => {
  ...
  const textStyle = {
    ...
    const handleDelete = (dataId) => {
      ...
      const index = cartItems.findIndex((item) => dataId === cartItems.id)
    ...
    //
    return(
        <div className='d-flex flex-wrap mt-3 justify-content-around container-fluid bg-warning'>
      ...
      {cartItems.map(data => (
        <div key={data.id} className=' container mb-3 '>
          <div className="d-flex shadow-sm border rounded-2 w-50 bg-body">
            <img className='img-thumbnail w-25' src={data.image} alt="" />
            <div className="d-flex flex-column m-2">
              <h3 className='mb-3'>{data.name}</h3>
              <h5>£{data.price}</h5>
              <p className='my-auto' style={lineTextStyle}>{data.desc}</p>
            ...
            <FontAwesomeIcon icon={faX} className="p-3" onClick={handleDelete}/>
          </div>
        </div>
      ))}
      </div>
      <div className='col'>
      <div className='card overflow-hidden mx-3 shadow col '>
        ...
        <div className={reveal ? "flex-column mb-3 p-1 text-center trans" : "collapse"}  id='collapsible' >
          {products.map(item => (<div className="d-flex justify-content-between border-bottom border-primary px-1 mb-3">
            ...
            <h5 className="m-0 d-flex gap-1 ">£{item.price}</h5>
          </div>
          ))}
        </div>
